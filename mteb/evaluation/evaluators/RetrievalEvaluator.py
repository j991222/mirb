...
class DenseRetrievalExactSearch:
    ...
    def search(
        ...
        **kwargs,
        ...
        print("Encoding queries complete")
        logger.info("Sorting Corpus by document length (Longest first)...")
        corpus_ids = sorted(
            corpus,
            key=lambda id: len(corpus[id]),
            reverse=True,
        )
        ...
        if excluded_ids:
            ...
            mapped_excluded_ids = {
                query_id: [inverted_index[excluded_id] for excluded_id in excluded_ids_list]
                for query_id, excluded_ids_list in excluded_ids.items()
            }
        ...
        for batch_num, corpus_start_idx in enumerate(itr):
            ...
            if excluded_ids:
                for i, qid in enumerate(query_ids):
                    mapped_excluded_ids_for_query = np.array(mapped_excluded_ids[qid])
                    mapped_excluded_ids_for_query_sub = mapped_excluded_ids_for_query[(mapped_excluded_ids_for_query >= corpus_start_idx) & (mapped_excluded_ids_for_query < corpus_end_idx)] - corpus_start_idx
                    similarity_scores[i, mapped_excluded_ids_for_query_sub] = -1
