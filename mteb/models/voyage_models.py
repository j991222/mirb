...
class VoyageWrapper(Wrapper):
    ...
    def _batched_encode(
        ...
        while index < len(sentences):
            ...
            while num_retries > 0:  # Cohere's API is not always reliable
                try:
                    batch_embeddings = self._embed_func(
                        texts=batch,
                    ...
                    print(f"Retrying... {num_retries} retries left.")
                    num_retries -= 1
                    if num_retries == 0:
                        raise e
            ...
            end_time = time.time()
            print(f"The time taken to encode a batch is {end_time - start_time} seconds")
