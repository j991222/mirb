...
class HFDataLoader:
    def __init__(
        ...
        excluded_ids_file: str = "excluded_ids.jsonl",
        qrels_folder: str = "qrels",
        qrels_file: str = "",
        dynamic_corpus: bool = False,
        ...
        if hf_repo:
            ...
            self.excluded_ids_file = (
                os.path.join(data_folder, excluded_ids_file) if data_folder else excluded_ids_file
            )
        self.dynamic_corpus = dynamic_corpus
    ...
    def load(
        self, split="test"
    ) -> tuple[dict[str, dict[str, str]], dict[str, str], dict[str, dict[str, int]], dict[str, Union[str, list[str]]]]:
        if not self.hf_repo:
        ...
        if self.dynamic_corpus and not len(self.excluded_ids):
        ...
        return self.corpus, self.queries, self.qrels, self.excluded_ids
    ...
    def _load_excluded_ids(self):
        if self.hf_repo:
            ...
            excluded_ids_ds = load_dataset(
                "json",
                data_files=self.excluded_ids_file,
                streaming=self.streaming,
                keep_in_memory=self.keep_in_memory,
            )
        excluded_ids_ds = next(iter(excluded_ids_ds.values()))  # get first split
        excluded_ids_ds = excluded_ids_ds.cast_column("query-id", Value("string"))
        self.excluded_ids = excluded_ids_ds
...
class AbsTaskRetrieval(AbsTask):
    ...
    def load_data(self, **kwargs):
        if self.data_loaded:
            return
        self.corpus, self.queries, self.relevant_docs, self.excluded_ids = {}, {}, {}, {}
        dataset_path = self.metadata_dict["dataset"]["path"]
        dynamic_corpus = self.metadata_dict["dataset"]["dynamic_corpus"]
        hf_repo_qrels = (
            dataset_path + "-qrels" if "clarin-knext" in dataset_path else None
        )
        for split in kwargs.get("eval_splits", self.metadata_dict["eval_splits"]):
            corpus, queries, qrels, excluded_ids = HFDataLoader(
                hf_repo=dataset_path,
                hf_repo_qrels=hf_repo_qrels,
                dynamic_corpus=dynamic_corpus,
                streaming=False,
            ...
            if dynamic_corpus:
                excluded_ids = {row['query-id']: row['excluded-ids'] for row in excluded_ids}
            self.corpus[split], self.queries[split], self.relevant_docs[split], self.excluded_ids[split] = (
                corpus,
                queries,
                qrels,
                excluded_ids,
        ...
        **kwargs,
        ...
        for hf_subset in hf_subsets:
            ...
            if hf_subset == "default":
                corpus, queries, relevant_docs, excluded_ids = (
                    self.corpus[split],
                    self.queries[split],
                    self.relevant_docs[split],
                    self.excluded_ids[split],
                )
            else:
                corpus, queries, relevant_docs, excluded_ids = (
                    ...
                    self.excluded_ids[hf_subset][split],
                )
            scores[hf_subset] = self._evaluate_subset(
                retriever, corpus, queries, relevant_docs, excluded_ids, hf_subset, **kwargs
            )
        return scores

    def _evaluate_subset(
        self, retriever, corpus, queries, relevant_docs, excluded_ids, hf_subset: str, **kwargs
    ) -> ScoresDict:
        start_time = time()
        results = retriever(corpus, queries, excluded_ids)
        end_time = time()
